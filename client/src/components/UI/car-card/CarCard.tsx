...
const CarCard = observer(({ car }: CarCardProps) => {
  const [isFavoriteCar, setIsFavoriteCar] = useState(!!favoritesStore.favorites.find(item => item.id === car.id));
  ...
  return (
    <div onClick={() => isStockCar(car.availability)} className={`car-card ${car.availability ? "" : "car-card--outstock"}`}>
      <Link to={`/${car.availability ? `cars/${car.id}` : ''}`} className="car-card__photo photo">
        {!car.availability && (
      ...
      <div className="car-card__info info">
        ...
        <div className="info__actions actions">
          ...
          <button onClick={onHandleClickFavorite} disabled={!car.availability} className={'actions__button button'}>
            {isFavoriteCar ? (
